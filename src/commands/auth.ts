...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  material-symbols-cli auth set sk-abc123xyz")
  .action((token: string) => {
    setToken(token);
...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  material-symbols-cli auth show\n  material-symbols-cli auth show --raw")
  .action((opts: { raw?: boolean }) => {
    if (!hasToken()) {
      log.warn("No token configured. Run: material-symbols-cli auth set <token>");
      return;
    }
...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  material-symbols-cli auth remove")
  .action(() => {
    removeToken();
...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  material-symbols-cli auth test")
  .action(async () => {
    try {
      await client.get("/");
